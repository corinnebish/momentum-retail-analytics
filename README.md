...
👉 View the live dashboard - no setup required, just click.
...
The Dashboard
An interactive Streamlit app with sidebar filters (date, category, region) driving three tabs. Explore it live here.
...
What does our customer base actually look like? (exploratory data analysis)
Which customers should we treat differently? (customer segmentation)
Where is revenue heading over the next six months? (forecasting)
Key Findings
...
File
What it does
explore.py, explore_charts.py
Exploratory analysis - data shape, date ranges, missing values, summary stats, and 4 charts (category/region sales, monthly trend, discount-vs-profit)
rfm_segmentation.py
Scores every customer on Recency/Frequency/Monetary, scales the features, and clusters them into 4 segments with K-Means
forecasting.py
Fits a Holt-Winters model (trend + 12-month seasonality) and forecasts the next 6 months of revenue
dashboard.py
Interactive Streamlit dashboard - sidebar filters (date, category, region) driving 3 tabs: Sales Overview, Customer Segments, Revenue Forecast
briefing.txt
The 5-minute business narrative, written for a non-technical audience - no jargon
customers_with_clusters.csv
Full segmentation output, one row per customer
...
Most people just want the live dashboard. If you'd rather run it yourself:
...
git clone https://github.com/corinnebish/momentum-retail-analytics.git

cd momentum-retail-analytics

uv sync

uv run streamlit run dashboard.py

The dashboard opens at http://localhost:8501. All data is included in the repo (Superstore.xlsx), so there's nothing else to configure.
